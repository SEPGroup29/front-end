...
export default function StockDetails({ fuelStation }) {
  ...
  return (
    <Box sx={{ flexGrow: 1, p: 5 }}>
      ...
      <ProgressBar completed={Math.floor((fuelStation.fuelStationId.rpstock / fuelStation.fuelStationId.pstock) * 100)} bgColor='#00897b' />
      <Typography mt={2} variant="h6" gutterBottom>
        Diesel {fuelStation.fuelStationId.rdstock}L remaining of {fuelStation.fuelStationId.dstock}L
      </Typography>
      <ProgressBar completed={Math.floor((fuelStation.fuelStationId.rdstock / fuelStation.fuelStationId.dstock) * 100)} bgColor='#00897b' />
    </Box>
  );
}
